...
def render_brand_slide(idx, slug, data, total=18):
...
<section class="slide slide--page{no_sub_class}">
  ...
  <div class="body">
    <div class="brand__grid" style="{'grid-template-columns:1fr' if not has_sub else ''}">
      {'<div class="brand__screen-wrap"><img class="brand__screen" src="' + screenshot_path + '" alt="' + brand_name + ' screenshot">' + annots_html + '<div class="brand__screen-caption"><span class="brand__screen-caption-url">' + url.replace("https://","").replace("http://","").rstrip("/") + '</span><span class="brand__screen-caption-type">' + screenshot_file.replace("_", " ").replace(".png", "") + '</span></div></div>' if has_sub else ''}
...
def main():
    ...
    for i, slug in enumerate(BRAND_ORDER):
        ...
        slides_html += render_brand_slide(4 + i, slug, data, total=18)
