...
async function getAccessToken(appKey, appSecret, authCode) {
    ...
    if (data.code !== 0) {
        throw new Error(`Token exchange failed: ${data.message} (code ${data.code})`);
    }
    const tokenData = data.data;
    console.error(`  Token length: ${tokenData.access_token.length} chars`);
    ...
    const expireIn = tokenData.access_token_expire_in ?? 0;
    return {
...
async function getAuthorizedShops(appKey, appSecret, accessToken) {
    ...
    if (data.code !== 0) {
        throw new Error(`Get shops failed: ${data.message} (code ${data.code})`);
    }
    // Handle different response formats
    const responseData = data.data ?? {};
    const shops = (responseData.shops ?? responseData.shop_list ?? []);
    return shops.map((s) => ({
...
async function doOAuthFlow(appKey, appSecret) {
...
async function main() {
    ...
    try {
        const shops = await getAuthorizedShops(appKey, appSecret, tokens.access_token);
        if (shops.length === 1) {
            ...
            shops.forEach((s, i) => console.log(`  ${i + 1}. ${s.shop_name} (${s.region})`));
