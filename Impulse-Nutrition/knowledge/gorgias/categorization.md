...
| `SAV_colis_bloqué` | Tracking loop, "jamais reçu", "bloqué en transit", Chronopost misrouted, pickup "held at location" | **Replacement draft Shopify** (sav_recipe.md) avec geste commercial (bidon/shaker) + ship **home address** (pas pickup point) + pickup manuel BigBlue après complete_draft + reply Gorgias. Référence `process_sav_unified.md` §2.1. |
...
| `SAV_produit_cassé_manquant` | "cassé", "abîmé", "endommagé", "manquant", "pas dans le colis", "fuite" | 1. BigBlue claim via `create_support_ticket` (en français). 2. Replacement draft si produit remplaçable (sav_recipe.md). 3. Reply confirmant au client. |
